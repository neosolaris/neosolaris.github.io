...
잠시 편집을 멈추고 쉘로 돌아갔다가 다시 vi로 돌아와야 할 때가 있다. 
...
설명한다. 쉘 명령어가 출력되면 vi에서 멈춤이 일어나고 아무 키를 누루면 vi로 돌아온다.
...
* `!!` 명령
	- 명령모드에서 `!!`를 사용하면 `awk`등으로 문자 치환을 할 수 있다.
	- 또는 snippet 프로그램 또는 스크립트를 만들어 스니팻 대용으로 쓸 수도 있다.
	- `:1,1!` ex명령이 하단에 찍힌다. 여기에 쉘명령을 넣으면 상호작용한다.
	- `:1,1!date` - 날짜를 해당 라인에 삽입
	- `:23,23!awk 'print($1)'` - 23번 줄의 첫 번째 단어로 대체
